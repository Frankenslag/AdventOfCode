...
namespace AdventOfCode2021
{
    internal class Day8
    {
        ...
        public static void Run()
        {
            ...
            {
                Dictionary<int, char[]> dctMapping = new Dictionary<int, char[]>();
                Dictionary<string, int> dctInvMapping = new Dictionary<string, int>();

                string[] aryInput = line.Split('|');
                List<string> aryDisplay =  aryInput[1].Split(' ').Select(s => s.ToCharArray().OrderBy(c => c).Select(c => c.ToString()).Aggregate((curr, next) => curr + next)).ToList();
                List<char[]> aryTest = aryInput[0].Split(' ').Select(s => s.ToCharArray().OrderBy(c => c).ToArray()).ToList();

                dctMapping[1] = aryTest.First(c => c.Length == 2);
                dctMapping[7] = aryTest.First(c => c.Length == 3);
                dctMapping[4] = aryTest.First(c => c.Length == 4);
                dctMapping[8] = aryTest.First(c => c.Length == 7);
                dctMapping[2] = aryTest.First(c => c.Length == 5 && c.Except(dctMapping[4]).Count() == 3);
                dctMapping[3] = aryTest.First(c => c.Length == 5 && c.Except(dctMapping[2]).Count() == 1);
                dctMapping[5] = aryTest.First(c => c.Length == 5 && c.Except(dctMapping[2]).Count() == 2);
                dctMapping[6] = aryTest.First(c => c.Length == 6 && c.Except(dctMapping[7]).Count() == 4);
                dctMapping[9] = aryTest.First(c => c.Length == 6 && c.Except(dctMapping[3]).Count() == 1);
                dctMapping[0] = aryTest.First(c => c.Length == 6 && c.Except(dctMapping[5]).Count() == 2);

                foreach (int key in dctMapping.Keys)
                {
                    dctInvMapping.Add(dctMapping[key].Select(c => c.ToString()).Aggregate((curr, next) => curr + next), key);
                }

                int dispVal = 0;

                foreach (string s in aryDisplay)
                {
                    dispVal *= 10;
                    dispVal += dctInvMapping[s];
                }

                cntCode += dispVal;

                cnt1478 += aryDisplay.Count(c => c.Length == 2 || c.Length == 4 || c.Length == 3 || c.Length == 7);
            }

            Console.WriteLine($"Day 8 Part 1 Answer is {cnt1478} values.");
            Console.WriteLine($"Day 8 Part 2 Answer is {cntCode} values.");
        }
