...
namespace AdventOfCode2015
{
    internal class Day16
    {
        ...
        {
            Regex rx = new Regex(@"Sue (?<id>\d+?): (?<item>.+?): (?<cnt>\d+?), (?<item>.+?): (?<cnt>\d+?), (?<item>.+?): (?<cnt>\d+?)");
            Dictionary<int, Dictionary<string, int>> sues = new Dictionary<int, Dictionary<string, int>>();
            Dictionary<string, int> mfcsam = new Dictionary<string, int>() { { "children", 3 }, { "cats", 7 }, { "samoyeds", 2 }, { "pomeranians", 3 }, { "akitas", 0 }, { "vizslas", 0 }, { "goldfish", 5 }, { "trees", 3 }, { "cars", 2 }, { "perfumes", 1 } };
            ...
            foreach (string line in data)
            {
                ...
                sues.Add(int.Parse(mc[0].Groups["id"].Value), new Dictionary<string, int>(mc[0].Groups["item"].Captures.Select((item, idx) => new KeyValuePair<string, int>(item.Value, int.Parse(mc[0].Groups["cnt"].Captures[idx].Value)))));
            }

            Console.WriteLine($"Day 16 Part 1 Answer is Sue{sues.Keys.First(id => sues[id].All(v => mfcsam.ContainsKey(v.Key) && mfcsam[v.Key] == v.Value))}.");
            Console.WriteLine(@$"Day 16 Part 2 Answer is Sue{sues.Keys.First(id => sues[id].All(v =>
            {
                return v.Key switch
                {
                    "cat" or "trees" => mfcsam.ContainsKey(v.Key) && mfcsam[v.Key] < v.Value,
                    "pomeranians" or "goldfish" => mfcsam.ContainsKey(v.Key) && mfcsam[v.Key] > v.Value,
                    _ => mfcsam.ContainsKey(v.Key) && mfcsam[v.Key] == v.Value
                };
            }))}.");
        }
    }
