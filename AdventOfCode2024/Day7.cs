...
namespace AdventOfCode2024
{
    internal class Day7
    {
        ...
        private static List<long> Process(List<long> input, bool part2)
        {
            List<long> retval = new();

            if (input.Count == 2)
            {
                retval.Add(input[0] * input[1]);
                retval.Add(input[0] + input[1]);
                if (part2)
                {
                    retval.Add(long.Parse($"{input[0]}{input[1]}"));
                }
            }
            else
            {
                foreach (var result in Process(input[..^1], part2))
                {
                    retval.Add(result * input[^1]);
                    retval.Add(result + input[^1]);
                    if (part2)
                    {
                        retval.Add(long.Parse($"{result}{input[^1]}"));
                    }
                }
            }
        ...
        public static void Run()
        {
            ...
            {
                IEnumerable<(long target, List<long> values)> calibrations = Data.Select(s => rx.Match(s)).Where(mx => mx.Success).Select(mx => (target: long.Parse(mx.Groups[1].Value), values: mx.Groups[2].Value.Split(' ').Select(long.Parse).ToList()));

                Console.WriteLine($"Day 7 Part {run + 1} Answer is calibration result {calibrations.Where(c => Process(c.values, run == 1).Any(v => v == c.target)).Sum(v => v.target)}.");
            }
        }
    }
}
