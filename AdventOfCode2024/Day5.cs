...
namespace AdventOfCode2024
{
    internal class Day5
    {
        private static readonly string[] PageOrderingRules =
        [
            "15|78",
            "65|46",
            "65|23",
        ...
        private static readonly string[] PageUpdates =
        [
        ...
        {
            bool TestUpdate(List<int> update, List<(int low, int high)> orderings) => update.Select((val, index) => (val, index)).All(page => !update[..page.index].Any(p => orderings.Contains(new ValueTuple<int, int>(page.val, p))) && !update[..page.index].Any(p => orderings.Contains(new(page.val, p))));

            IEnumerable<List<int>> updates = PageUpdates.Select(v => v.Split(',').Select(int.Parse).ToList());

            List<(int low, int high)> orderings = PageOrderingRules.Aggregate(new List<(int low, int high)>(), (c, n) => { string[] ary = n.Split('|'); c.Add(new(int.Parse(ary[0]), int.Parse(ary[1]))); return c; });

            Console.WriteLine($"Day 5 Part 1 Answer is {updates.Where(update => TestUpdate(update, orderings)).Select(update => update[update.Count / 2]).Sum()}.");
            Console.WriteLine($"Day 5 Part 2 Answer is {updates.Where(update => !TestUpdate(update, orderings)).Select(update => update.OrderBy(e => e, Comparer<int>.Create((x, y) => orderings.Contains(new(x, y)) ? -1 : orderings.Contains(new(y, x)) ? 1 : 0)).ToList()).Select(update => update[update.Count / 2]).Sum()}.");
        }
    }
}
