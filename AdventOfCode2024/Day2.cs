...
namespace AdventOfCode2024
{
    internal class Day2
    {
        ...
        private static bool TestArray(IEnumerable<(int val, int _)> report)
        {
            int[] ary = report.Select(rep => rep.val).ToArray();

            return ary.Zip(ary.Skip(1)).All(v => v.Second - v.First >= 1 && v.Second - v.First <= 3) || ary.Zip(ary.Skip(1)).All(v => v.First - v.Second >= 1 && v.First - v.Second <= 3);
        }
        ...
        public static void Run()
        {
            ...
            for (int run = 0; run < 2; run++)
            {
                ...
                {
                    IEnumerable<(int val, int idx)> report = strLine.Split(' ', StringSplitOptions.RemoveEmptyEntries).Select((s, idx) => (val: int.Parse(s), idx)).ToList();
                    ...
                    {
                        if (Enumerable.Range(0, report.Count()).Select(remIdx => report.Where(vi => vi.idx != remIdx)).ToList().Any(TestArray))
                        {
                            cnt++;
                        }
                    }
                }
                Console.WriteLine($"Day 2 Part {run + 1} Answer is {cnt} reports are safe.");
            }
