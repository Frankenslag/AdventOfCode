...
namespace AdventOfCode2024
{
    internal class Day13
    {
        ...
        private class Claw
        {
            public Claw((long x, long y) buttonA, (long x, long y) buttonB, (long x, long y) prize)
            {
                ButtonA = buttonA;
                ButtonB = buttonB;
                Prize = prize;
            ...
            public (decimal a, decimal b) Presses
            {
                get 
                {
                    decimal aPresses = (Prize.y - ButtonB.y * (decimal) Prize.x / ButtonB.x) / (ButtonA.y - ButtonB.y * (decimal) ButtonA.x / ButtonB.x);
                    return (aPresses, (Prize.x - aPresses * ButtonA.x) / ButtonB.x);
                }
            }
        }
        ...
        {
            Regex rx = new(@".+:\s+X[+=]([0-9]+),\s+Y[+=]([0-9]+)");

            List<Claw> claws = inputdata.Select((line, idx) => (line, idx)).GroupBy(l => l.idx / 4).Select(grp => grp.Select(l => l.line).Where(l => !string.IsNullOrEmpty(l)).Select((l, index) =>
            {
                Match mx = rx.Match(l);
                return (index, x: long.Parse(mx.Groups[1].Value), y: long.Parse(mx.Groups[2].Value));
            }).Aggregate(new Claw(), (curr, next) =>
            {
                switch (next.index)
                {
                    case 0:
                        curr.ButtonA = (next.x, next.y);
                        break;
                    case 1:
                        curr.ButtonB = (next.x, next.y);
                        break;
                    case 2:
                        curr.Prize = (next.x, next.y);
                        break;
                }
            ...
            {
                Console.WriteLine($"Day 13 Part {part} Answer is {Math.Round(claws.Select(c => new Claw(c.ButtonA, c.ButtonB, (c.Prize.x + (part == 0 ? 0 : 10000000000000), c.Prize.y + (part == 0 ? 0 : 10000000000000))).Presses).Where(presses => presses is {a: > 0M, b: > 0M} && Math.Abs(presses.a - decimal.Round(presses.a)) < 0.00001M && Math.Abs(presses.b - decimal.Round(presses.b)) < 0.00001M).Sum(presses => presses.a * 3 + presses.b)):N0} tokens.");
            }
        }
