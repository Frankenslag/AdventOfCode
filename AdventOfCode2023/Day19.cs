...
namespace AdventOfCode2023
{
    internal class Day19
    {
        ...
        private enum WorkflowRuleCompare
        {
            LessThan = '<',
            MoreThan = '>'
        }
        ...
        private record Workflow(string Dflt, List<WorkflowRule> Rules);
        private record WorkflowRule(PartCategory Category, WorkflowRuleCompare Compare, int CompareTo, string ResultantWorkflow);

        private record PartIntervals(Interval X, Interval M, Interval A, Interval S)
        {
            public bool IsEmpty => X.Length <= 0 && M.Length <= 0 && A.Length <= 0 && S.Length <= 0;

            public Interval this[PartCategory category]
            {
                get => category switch
                {
                    PartCategory.CatX => X,
        ...
        private static int Part1(List<Dictionary<PartCategory, int>> parts, Dictionary<string, Workflow> workflows)
        {
            bool TestPart(Dictionary<PartCategory, int> part)
            {
                string NextWorkflow(Dictionary<PartCategory, int> part, string workflowName) =>
                    workflows[workflowName].Rules.FirstOrDefault(rule => (rule.Compare == WorkflowRuleCompare.LessThan && part[rule.Category] < rule.CompareTo) || (rule.Compare == WorkflowRuleCompare.MoreThan && part[rule.Category] > rule.CompareTo))?.ResultantWorkflow ?? workflows[workflowName].Dflt;

                string currWorkflow = NextWorkflow(part, "in");

                while (currWorkflow is not ("A" or "R"))
                {
                    currWorkflow = NextWorkflow(part, currWorkflow);
                }

                return currWorkflow == "A";
            }


            return parts.Sum(part => TestPart(part) ? part.Values.Sum() : 0);
        }

        private static int Part2(Dictionary<string, Workflow> workflows)
        {
            PartIntervals currInterval = new(new Interval(1, 4000), new Interval(1, 4000), new Interval(1, 4000), new Interval(1, 4000));
            Workflow currWorkflow = workflows["in"];


            foreach (WorkflowRule rule in currWorkflow.Rules)
            {
                var propertyVal = currInterval[rule.Category];

                if (rule.Compare == WorkflowRuleCompare.LessThan)
                {
                }
                else
                {
        ...
        public static void Run()
        {
            ...
            Regex rxWorkflow = new(@"(?<name>[a-z]+)\{((?<rule>[xmas][\<\>]\d+:[a-z|R|A]+),)+?(?<default>[a-z|R|A]+)\}");
            Regex rxRule = new(@"(?<category>[xmas])(?<compare>[\<\>])(?<compareto>\d+):(?<result>[a-z|R|A]+)");
            Regex rxPart = new(@"(?<category>[xmas])\=(?<value>\d+)");

            Dictionary<string, Workflow> workflows = data.TakeWhile(line => !string.IsNullOrEmpty(line)).Select(line => rxWorkflow.Match(line)).ToDictionary(mx => mx.Groups["name"].Value, mx =>
                new Workflow(mx.Groups["default"].Value, mx.Groups["rule"].Captures.Aggregate(new List<WorkflowRule>(), (curr, next) =>
                {
                    var mxRule = rxRule.Match(next.Value);
                    if (mxRule.Success)
                    {
                        curr.Add(new WorkflowRule((PartCategory)mxRule.Groups["category"].Value[0], (WorkflowRuleCompare)mxRule.Groups["compare"].Value[0], int.Parse(mxRule.Groups["compareto"].Value), mxRule.Groups["result"].Value));
                    }
                    return curr;
                }))
            );

            List<Dictionary<PartCategory, int>> parts = data.Skip(workflows.Count + 1).Select(line => rxPart.Matches(line).ToDictionary(mx => (PartCategory)mx.Groups["category"].Value[0], mx => int.Parse(mx.Groups["value"].Value))).ToList();

            Console.WriteLine($"Day 19 Part 1 Answer is {Part1(parts, workflows)}.");
        }
    }
}
