...
namespace AdventOfCode2023
{
    internal class Day2
    {
        ...
        {
            Regex rx1 = new(@"Game (\d+): (.*)");
            Regex rx2 = new(@"(\d+) \b(red|green|blue)\b");
            long sum1 = 0;
            long sum2 = 0;

            foreach (string line in Data)
            {
                Match mx = rx1.Match(line);
                if (mx is {Success: true, Groups.Count: 3})
                {
                    Dictionary<string, int> bag = new() {{"red", 12},{"green", 13}, {"blue", 14}};

                    if (mx.Groups[2].Value.Split(new[] {';'}, StringSplitOptions.TrimEntries).All(set => set.Split(new[] { ',' }, StringSplitOptions.TrimEntries).All(
                            grab =>
                            {
                                Match mx2 = rx2.Match(grab);
                                return bag[mx2.Groups[2].Value] - int.Parse(mx2.Groups[1].Value) >= 0;
                            })))
                    {
                        sum1 += int.Parse(mx.Groups[1].Value);
                    }

                    Dictionary<string, int> minBag = new() { { "red", 0 }, { "green", 0 }, { "blue", 0 } };

                    foreach (string set in mx.Groups[2].Value.Split(new[] {';'}, StringSplitOptions.TrimEntries))
                    {

                        foreach (string grab in set.Split(new[] {','}, StringSplitOptions.TrimEntries))
                        {
                            Match mx2 = rx2.Match(grab);

                            minBag[mx2.Groups[2].Value] = int.Max(minBag[mx2.Groups[2].Value], int.Parse(mx2.Groups[1].Value));
                        }

                    }

                    sum2 += minBag["red"] * minBag["green"] * minBag["blue"];
                }
            }
            Console.WriteLine($"Day 2 Part 1 Answer is sum value {sum1}.");
            Console.WriteLine($"Day 2 Part 2 Answer is sum value {sum2}.");
        }
    }
}
