...
namespace AdventOfCode2023
{
    internal class Day10
    {
        ...
        private static readonly Dictionary<char, PipeType> CharToPipe = new() {{'.', PipeType.None}, { '|', PipeType.Vertical }, { '-', PipeType.Horizontal },
            { 'L', PipeType.NorthEast }, { 'J', PipeType.NorthWest }, { '7', PipeType.SouthEast }, { 'F', PipeType.SouthWest }, { 'S', PipeType.Start }};

        private static List<(int col, int row)> FindAdjacentPipes((int col, int row) currPipe, PipeType[,] pipeMap)
        {
            List < (int col, int row)> retval = [];

            // above currPipe
            if (currPipe.row > 0 && pipeMap[currPipe.col, currPipe.row] is PipeType.Start or PipeType.Vertical or PipeType.NorthEast or PipeType.NorthWest)
            {
                if (pipeMap[currPipe.col, currPipe.row - 1] is PipeType.Vertical or PipeType.SouthEast or PipeType.SouthWest)
                {
                    retval.Add((currPipe.col, currPipe.row - 1));
                }
            }

            // below currPipe
            if (currPipe.row < pipeMap.GetLength(1) - 1 && pipeMap[currPipe.col, currPipe.row] is PipeType.Start or PipeType.Vertical or PipeType.SouthEast or PipeType.SouthWest)
            {
                if (pipeMap[currPipe.col, currPipe.row + 1] is PipeType.Vertical or PipeType.NorthEast or PipeType.NorthWest)
                {
                    retval.Add((currPipe.col, currPipe.row + 1));
                }
            }
            ...
            // left of currPipe
            if (currPipe.col > 0 && pipeMap[currPipe.col, currPipe.row] is PipeType.Start or PipeType.Horizontal or PipeType.NorthWest or PipeType.SouthEast)
            {
                if (pipeMap[currPipe.col -1, currPipe.row] is PipeType.Horizontal or PipeType.NorthEast or PipeType.SouthWest)
                {
                    retval.Add((currPipe.col - 1, currPipe.row));
                }
            }

            // right of currPipe
            if (currPipe.col < pipeMap.GetLength(0) - 1 && pipeMap[currPipe.col, currPipe.row] is PipeType.Start or PipeType.Horizontal or PipeType.NorthEast or PipeType.SouthWest)
            {
                if (pipeMap[currPipe.col + 1, currPipe.row] is PipeType.Horizontal or PipeType.NorthWest or PipeType.SouthEast)
                {
                    retval.Add((currPipe.col + 1, currPipe.row));
                }
            }
        ...
        public static void Run()
        {
            ...
            (int col, int row) startPoint = (-1, -1);

            PipeType[,] pipeMap = data.SelectMany((line, rowIdx) => line.Select((c, colIdx) => (pipe: CharToPipe[c], colIdx, rowIdx))).Aggregate(new PipeType[data[0].Length, data.Length], (curr, next) =>
            {
                curr[next.colIdx, next.rowIdx] = next.pipe;

                if (next.pipe == PipeType.Start)
                {
                    startPoint = (next.colIdx, next.rowIdx);
                }

                return curr;
            });

            (int col, int row) currPoint = startPoint;
            (int col, int row) lastPoint = (-1, -1);
            List<(int col, int row )> pipePath = [startPoint];

            do
            {

                (int col, int row) nextPoint = FindAdjacentPipes(currPoint, pipeMap).FirstOrDefault(p => p != lastPoint, (-1, -1));

                if (nextPoint.col != -1)
                {
                    pipePath.Add(nextPoint);
                    lastPoint = currPoint;
                }

                currPoint = (nextPoint.col, nextPoint.row);
            } while (currPoint.col != -1 && currPoint != startPoint);

            Console.WriteLine($"Day 10 Part 1 Answer is {Math.Ceiling((decimal)pipePath.Count / 2)} steps.");

            pipePath.Add(startPoint);

            // shoelace alogorithm to get the area
            int area = pipePath.Zip(pipePath.Skip(1)).Sum(t => t.First.row * t.Second.col - t.Second.row * t.First.col ) / 2;

            // pick algorithm to find the count of internal points
            int points = (int)(Math.Abs(area) - 0.5 * (pipePath.Count - 1) + 1);

            Console.WriteLine($"Day 10 Part 2 Answer is {points} steps.");
        }
    }
}