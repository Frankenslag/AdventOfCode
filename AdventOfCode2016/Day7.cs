...
namespace AdventOfCode2016
{
    internal class Day7
    {
        ...
        private static bool HasAbba(string str) => Regex.Matches(str, @"(?<outer>[a-z])(?<inner>[a-z])\k<inner>\k<outer>").Any(mx => mx.Success && mx.Groups["inner"].Value != mx.Groups["outer"].Value);
        ...
        public static void Run()
        {
            ...
            {
                string[] supernets = Regex.Replace(line, @"\[.*?\]", ",").Split(',').Distinct().ToArray();
                string[] hypernets = Regex.Matches(line, @"\[(.*?)\]").Where(mx => mx.Success).Select(mx => mx.Groups[1].Value).Distinct().ToArray();

                if (supernets.Any(HasAbba) && !Regex.Matches(line, @"\[(.*?)\]").Where(mx => mx.Success).Select(mx => mx.Groups[1].Value).Any(HasAbba))
                {
                    cntTLS++;
                }

                string[] abas = supernets.SelectMany(net => Regex.Matches(net, @"(?=(?<outer>[a-z])(?<inner>[a-z])\k<outer>)").Where(mx => mx.Success && mx.Groups["inner"].Value != mx.Groups["outer"].Value).Select(mx => $"{mx.Groups["outer"].Value}{mx.Groups["inner"].Value}")).ToArray();
                string[] babs = hypernets.SelectMany(net => Regex.Matches(net, @"(?=(?<outer>[a-z])(?<inner>[a-z])\k<outer>)").Where(mx => mx.Success && mx.Groups["inner"].Value != mx.Groups["outer"].Value).Select(mx => $"{mx.Groups["inner"].Value}{mx.Groups["outer"].Value}")).ToArray();

                if (abas.Any(aba => babs.Any(bab => bab == aba)))
                {
                    cntSSL++;
                }
            }

            Console.WriteLine($"Day 7 Part 1 Answer is {cntTLS} addresses support TLS.");
            Console.WriteLine($"Day 7 Part 2 Answer is {cntSSL} addresses support SSL.");
        }
    }
}
