    ...
    internal class Day4
    {
        ...
        public static void Run()
        {
            ...
            foreach (string line in Data)
            {
                ...
                if (mx.Success)
                {
                    string encryptedRoomName = mx.Groups[1].Value;

                    string calculatedChecksum = encryptedRoomName.ToCharArray().Where(c => c != '-').GroupBy(c => c).Select(g => new { ch = g.Key, cnt = g.Count() }).OrderByDescending(c => c.cnt).ThenBy(c => c.ch).Select(c => c.ch).Aggregate(string.Empty, (curr, next) => curr + next).Substring(0, 5);

                    if (calculatedChecksum == mx.Groups[3].Value)
                    {
                        int sectorId = int.Parse(mx.Groups[2].Value);
                        sum += sectorId;

                        var t =Encoding.ASCII.GetString(Encoding.ASCII.GetBytes(encryptedRoomName).Select(b => (byte)(b == 0x2d ? 0x20 : ((b - 0x61 + sectorId) % 26) + 0x61)).ToArray());

                        Console.WriteLine(t);

                        if (t == "northpole object storage")
                        {
                            resultPart2 = sectorId;
                        }
                    }
                }
            }

            Console.WriteLine($"Day 4 Part 1 Answer Sum of sector ID's is {sum}.");
            Console.WriteLine($"Day 4 Part 2 Answer Sum of sector ID's is {resultPart2}.");
        }
    }
}
